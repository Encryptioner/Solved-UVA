...
int main()
{
    ...
    long long unsigned int b[3],c[3],g[3],ans[6],i,m=0,x;

    char req[][5]={{"BCG"},{"BGC"},{"CBG"},{"CGB"},{"GBC"},{"GCB"}};

    while(cin>>b[0]>>g[0]>>c[0]>>b[1]>>g[1]>>c[1]>>b[2]>>g[2]>>c[2])
    {
            ans[0]=b[1]+b[2]+c[0]+c[2]+g[0]+g[1];
            ans[1]=b[1]+b[2]+g[0]+g[2]+c[0]+c[1];

            ans[2]=c[1]+c[2]+b[0]+b[2]+g[0]+g[1];
            ans[3]=c[1]+c[2]+g[0]+g[2]+b[0]+b[1];

            ans[4]=g[1]+g[2]+b[0]+b[2]+c[0]+c[1];
            ans[5]=g[1]+g[2]+c[0]+c[2]+b[0]+b[1];

            for(i=0,m=ans[0],x=0;i<6;i++)
            {
                if( ans[i] < m)
                {
                    m=ans[i];
